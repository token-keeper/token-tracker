...
> **⚠️ 스크립트 부재 안내**: 이 문서가 설명하는 `scripts/dev-mode.sh` 는 현재 레포에 **존재하지 않는다**. 공개 정리 PR 에서 제거됐다. 다시 쓰려면 먼저 git history 에서 스크립트 본문을 복구해야 한다:
>
> ```bash
> # 마지막 dev-mode.sh 가 있던 커밋 찾기
> git log --all --oneline -- scripts/dev-mode.sh
> # 해당 커밋에서 파일만 복구
> git checkout <commit-hash> -- scripts/dev-mode.sh
> chmod +x scripts/dev-mode.sh
> ```
>
> 이 문서는 공개 README 에서 빠진 내용이지만, **repo 의 `docs/internal/` 자체는 public 영역**이라 외부에서도 보인다. 시크릿/토큰은 포함되지 않으며 노출된 경로(`~/.claude/plugins/cache/...`)는 Claude Code 의 일반 cache 레이아웃이다.
...
- 이 문서는 본인 참고용 매뉴얼이지만 repo 의 `docs/internal/` 은 public 영역이라 외부에서도 보인다. 시크릿이 들어가지 않도록 주의.
- `scripts/dev-mode.sh` 가 다시 필요해지면 상단의 git checkout 명령으로 복구한다.
